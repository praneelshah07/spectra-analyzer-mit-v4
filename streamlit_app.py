...
# Left and Right Column Layout for the Streamlit App
left_column, right_column = st.columns(2)

# Left Column - Parts 1-5: Configuration Options
with left_column:
    ...
    # Part 3: Functional Group Labels
    st.subheader("Functional Group Labels")
    ...
    with st.form(key='functional_group_form'):
        ...
        fg_wavelength = st.number_input("Wavelength Position (µm)", min_value=3.0, max_value=20.0, value=12.4)
    ...
    # Display functional group labels
    ...
    # Part 4: Plot Settings
    st.subheader("Plot Settings")
    bin_type = st.selectbox('Select binning type:', ['None', 'Wavelength (in microns)'])
    bin_size = st.number_input('Enter bin size in microns:', value=0.1)
    ignore_q_branch = st.checkbox('Ignore Q-branch for normalization')

    # Part 5: Peak Detection
    peak_finding_enabled = st.checkbox('Enable Peak Finding', value=False)
    num_peaks = st.slider('Number of Prominent Peaks to Detect', min_value=1, max_value=10, value=5)

    # Confirmation Button
    confirm_button = st.button('Confirm Selection and Plot')

# Right Column - Plot Section
with right_column:
    st.header("Plot Output")

    if confirm_button:
        st.write("Plot will be generated here based on your selections.")
        ...
        for smiles, spectra in data[data['SMILES'].isin(filtered_smiles)][['SMILES', 'Raw_Spectra_Intensity']].values:
            if bin_type != 'None':
                spectra, x_axis = bin_and_normalize_spectra(spectra, bin_size, bin_type.lower(), q_branch_threshold=0.8 if ignore_q_branch else None)
            else:
                spectra = spectra / np.max(spectra)  # Normalize if no binning
                x_axis = wavelength
            ax.fill_between(x_axis, 0, spectra, color=random.choice(color_options), alpha=0.5, label=smiles)

            if peak_finding_enabled:
                peaks, properties = find_peaks(spectra, height=0.05, prominence=0.1)
                top_peaks = sorted(peaks[:num_peaks])
                for peak in top_peaks:
                    peak_wavelength = x_axis[peak]
                    peak_intensity = spectra[peak]
                    ax.text(peak_wavelength, peak_intensity + 0.05, f'{round(peak_wavelength, 1)}', fontsize=10, ha='center', color='black')
        ...
        ax.set_xticks([3, 4, 5, 6, 7, 8, 9, 11, 12, 15, 20])
